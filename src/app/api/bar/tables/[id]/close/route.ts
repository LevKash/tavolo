...
import { closeTableSession } from "@/lib/core";
import { checkBarAccess } from "@/lib/pin";

export const dynamic = "force-dynamic";

/** Close a table after payment at the cash desk. */
export async function POST(
  ...
  const closed = await closeTableSession(venue.id, id);
  return NextResponse.json({ ok: true, closed: Boolean(closed) });
}
